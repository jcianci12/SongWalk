...
- enables Quick Tunnel by default in the packaged desktop runtime unless you explicitly set `SONGWALK_QUICK_TUNNEL_ENABLED=0`
...
For the packaged app, the default data directory is `songwalk-data/` next to `SongWalk.exe`, which keeps the distribution portable when you move the whole folder together. Set `SONGWALK_DATA_DIR` only if you want a fixed external location.
...
- If you want a fixed data location, set `SONGWALK_DATA_DIR` before launching the packaged app.
