...
def ensure_portable_data_dir() -> Path | None:
    configured = os.getenv("SONGWALK_DATA_DIR", "").strip()
    if configured:
    ...
    data_dir = Path(sys.executable).resolve().parent / "songwalk-data"
    os.environ["SONGWALK_DATA_DIR"] = str(data_dir)
    return data_dir
...
def prepare_runtime(test_config: dict | None = None) -> PreparedRuntime:
    ...
    host = os.getenv("SONGWALK_HOST", "0.0.0.0")
    port = int(os.getenv("SONGWALK_PORT", "8080"))
...
def resolve_quick_tunnel_enabled() -> bool:
    configured = os.getenv("SONGWALK_QUICK_TUNNEL_ENABLED", "").strip().lower()
...
def resolve_cloudflared_binary() -> str:
    configured = os.getenv("SONGWALK_CLOUDFLARED_BIN", "").strip()
    if configured:
        return configured
