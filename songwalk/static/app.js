(function () {
  ...
  if (uploadForm && hiddenFileInput) {
    ...
    function setUploadProgress(loaded, total) {
      ...
      uploadProgressCopy.textContent = total > 0
        ? `${percent}% Ã‚Â· ${formatBytes(loaded)} / ${formatBytes(total)}`
        : `${percent}%`;
    }
    ...
    row.addEventListener("dragover", (event) => {
      ...
      if (draggedRow && draggedRow !== row) {
        ...
        if (targetNext === draggedRow) {
          // Dragged is already right after target Ã¢â‚¬â€ swap them
  ...
  // ---- Bridge exports for sync.js ----
  window.__songwalk = window.__songwalk || {};
  window.__songwalk.selectRow = selectRow;
  window.__songwalk.findRowByTrackId = findRowByTrackId;
  window.__songwalk.trackStateFromRow = trackStateFromRow;
  window.__songwalk.currentPlaybackRow = currentPlaybackRow;
  window.__songwalk.refreshLibraryAfterMutation = refreshLibraryAfterMutation;
  window.__songwalk.player = player;
  window.__songwalk.nextButton = nextButton;
  window.__songwalk.prevButton = prevButton;
  window.__songwalk.progressInput = progressInput;
  window.__songwalk.escapeHtml = escapeHtml;
  Object.defineProperty(window.__songwalk, 'isReloadingForSharedLibraryState', {
    get: function () { return isReloadingForSharedLibraryState; },
    configurable: true
  });
})();
